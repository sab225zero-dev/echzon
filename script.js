...
if (window.innerWidth <= 768) {
    document.querySelectorAll(".reveal, .reveal-item, .story-section, .timeline-item").forEach(el => {
...
numbers.forEach(item=>{counterObserver.observe(item);});
...
MAGNETIC BUTTONS
=========================================*/
document.querySelectorAll(".main-btn,.secondary-btn").forEach(button=>{
    button.addEventListener("mousemove",(e)=>{
        const rect=button.getBoundingClientRect();
        const x=e.clientX-rect.left-rect.width/2;
        const y=e.clientY-rect.top-rect.height/2;
        button.style.transform=`translate(${x*.18}px,${y*.18}px)`;
    });
        ...
        for(let a=0;a<particles.length;a++){
            for(let b=a+1;b<particles.length;b++){
                ...
                if(distance<130){
                    ...
                    ctx.strokeStyle=`rgba(47,214,255,${1-distance/130})`;
                    ctx.lineWidth=.4;
                    ctx.stroke();
                }
            }
...
if (cta) {
    window.addEventListener("scroll",()=>{
        ...
        scrollTimer=setTimeout(()=>{ cta.classList.add("pulse"); },2000);
    });
}

// --- تفعيل مشغلات الصوت المخصصة (تم الإصلاح والربط المباشر) ---
    ...
    if (playBtn && audio) {
        playBtn.addEventListener('click', () => {
            ...
            document.querySelectorAll('.audio-element').forEach(otherAudio => {
                if (otherAudio !== audio) {
                    otherAudio.pause();
                    const otherPlayer = otherAudio.closest('.custom-player');
                    if (otherPlayer) {
                        const otherIcon = otherPlayer.querySelector('.play-pause-btn i');
                        const otherStatus = otherPlayer.querySelector('.player-status');
                        if (otherIcon) otherIcon.className = 'fas fa-play';
                        if (otherStatus) otherStatus.textContent = 'تشغيل العينة';
                    }
                }
            });
